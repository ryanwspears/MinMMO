...
async function readConfig() {
    const { rows } = await pool.query('SELECT value FROM game_config WHERE key = $1', [CONFIG_KEY]);
    if (rows.length === 0) {
...
async function writeConfig(next) {
    ...
    await pool.query('INSERT INTO game_config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()', [CONFIG_KEY, repaired]);
    return repaired;
}
