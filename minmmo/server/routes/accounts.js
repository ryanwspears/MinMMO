...
router.get('/', async (_req, res, next) => {
    try {
        const { rows } = await pool.query(`SELECT a.id, a.active_character_id, a.created_at, a.updated_at, COUNT(c.id) AS character_count
...
router.post('/', async (req, res, next) => {
    ...
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO account_credentials (id, password_hash)
       VALUES ($1, $2)
       RETURNING id, active_character_id, created_at, updated_at, 0 AS character_count`, [id, hashed]);
        res.status(201).json({
            id: rows[0].id,
            characters: {},
...
router.post('/authenticate', async (req, res, next) => {
    ...
    try {
        const { rows } = await pool.query('SELECT password_hash FROM account_credentials WHERE id = $1', [id]);
        if (rows.length === 0) {
...
router.post('/:accountId/selection', async (req, res, next) => {
    ...
    try {
        ...
        await pool.query(`UPDATE account_credentials
       SET active_character_id = $2,
           updated_at = NOW()
       WHERE id = $1`, [accountId, characterId]);
        if (characterId) {
            const { rowCount } = await pool.query(`UPDATE account_characters
         SET last_selected_at = NOW(),
             updated_at = NOW()
         WHERE id = $1 AND account_id = $2`, [characterId, accountId]);
            if (rowCount === 0) {
...
router.get('/:accountId/characters', async (req, res, next) => {
    ...
    try {
       ...
       ORDER BY COALESCE(c.last_selected_at, to_timestamp(0)) DESC, c.updated_at DESC`, [accountId]);
...
router.get('/:accountId/characters/:characterId', async (req, res, next) => {
    ...
    try {
       ...
       LEFT JOIN character_world_states w ON w.character_id = c.id
       LEFT JOIN character_inventories i ON i.character_id = c.id
       WHERE c.account_id = $1 AND c.id = $2`, [accountId, characterId]);
        if (rows.length === 0) {
