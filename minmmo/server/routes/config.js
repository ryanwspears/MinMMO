...
router.post('/', async (req, res, next) => {
    ...
    try {
        const { rows } = await pool.query('INSERT INTO game_config (key, value) VALUES ($1, $2) RETURNING key, value', [key, value]);
...
router.put('/:key', async (req, res, next) => {
    ...
    try {
        const { rows } = await pool.query('UPDATE game_config SET value = $2, updated_at = NOW() WHERE key = $1 RETURNING key, value', [req.params.key, value]);
        if (rows.length === 0) {
            res.status(404).json({ message: 'Config not found' });
            return;
        }
